...
local_ip=`echo $local_storage_mgmt_ip | cut -d / -f1`
if [ $compute_storage_mgmt_ip == $local_ip ]; then
   # compute role
   for ip in $controllers_storage_mgmt_ips; do
       ovs-vsctl add-port br0 vxlan_$ip --set interface vxlan_$ip type=vxlan options:remote_ip=$ip
   done
else
   # controller
   ovs-vsctl add-port br0 vxlan_$compute_storage_mgmt_ip --set interface vxlan_$compute_storage_mgmt_ip type=vxlan options:remote_ip=$compute_storage_mgmt_ip
   for ip in $controllers_storage_mgmt_ips; do
       if [ $ip != $local_ip ]; then
           ovs-vsctl add-port br0 vxlan_$ip --set interface vxlan_$ip type=vxlan options:remote_ip=$ip
   done
